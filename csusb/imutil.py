...
def imshow(image, ext = 0, vmin = 0, vmax = 0):
    """
    Display an image.
    ...
    if image.ndim == 2:
        ...
        if ext < nframes:
            ...
            plt.imshow(image[ext,:,:], origin = 'lower', vmin = vmin, vmax = vmax)
            plt.colorbar()
            plt.axis("off")
            plt.show()
        else:
...
def imcombine(img, combine = "average", nframes = 100):
    """
    Combine image frames of CSUSB telescope image cube.
    
    Parameters
    ----------
    img : numpy array
        CSUSB telescope image cube
