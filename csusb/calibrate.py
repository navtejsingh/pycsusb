...
def masterbias(image, skip = 0):
    ...
    ----------
    image : numpy array
        Bias array from CSUSB telescope
...
def masterdark(image, skip = 0):
    """
    Create an average master dark frame.
    
    Parameters
    ----------
    image : numpy array
        Dark array from CSUSB
    ...
    """
    if image.ndim == 3:
        master_dark = np.median(image[skip:,:,:], axis = 0)
        return master_dark
    else:
        print "MASTERDARK: Only 3D image arrays are supported"
        
    return
    
        	    	
def masterflat(flat_image, dark_image, threshold = 0.8):	
    """
    Generate normalized flat field.
    ...
    dark_image : numpy array
        2D master dark image
    ...
    if flat_image.ndim == 3 and dark_image.ndim == 2:
        ...
        print "MASTERFLAT: Only 3D flat field and 2D master dark supported."
        return
...
def dedark(sci_image, dark_image):
    """
    De-bias science frames.
    ...
    debias_sci_image : numpy array
        Debias science image
    """
    if dark_image.ndim == 2 and (sci_image.ndim == 2 or sci_image.ndim == 3):
        debias_sci_image = sci_image - dark_image
        return debias_sci_image
    else:
        print "DEBIAS: Only 2D master bias and 2D/3D science images are supported."
        return
